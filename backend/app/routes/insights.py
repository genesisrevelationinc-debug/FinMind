...
def weekly_summary():
    ...
    summary = {
        ...
        "expenses": [{"id": expense.id, "amount": expense.amount, "category": expense.category, "notes": expense.notes, "date": expense.date.isoformat()} for expense in expenses],
        "bills": [{"id": bill.id, "name": bill.name, "amount": bill.amount, "cadence": bill.cadence, "due_date": bill.due_date.isoformat(), "channel": bill.channel} for bill in bills]
    }
